...
document.addEventListener('scroll', function() {
    ...
    else if (window.scrollY > thresholdOne && window.scrollY <= thresholdTwo) // 1 x VH
    {
        secondPane.style.left = ((((2 * viewportHeight - window.scrollY) / viewportHeight) - 1) * 100) + '%';
        secondPane.style.top = window.scrollY + 'px';
        lastPane.style.top = window.scrollY + viewportHeight + 'px';
    }
    else if (window.scrollY > thresholdTwo) // 2 x VH
    {
        secondPane.style.left = - viewportWidth + 'px';
    }
});